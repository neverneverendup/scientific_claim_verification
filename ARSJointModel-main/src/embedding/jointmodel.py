...
class ArgSentenceAttention(nn.Module):
    """
    sentence-level attention. paragraph representation for label prediction.
    """
    def __init__(self, input_size, output_size, dropout=0.1):
        super(ArgSentenceAttention, self).__init__()
        self.sentence_attention = TimeDistributed(input_size+6, output_size)
        self.activation = torch.tanh
        self.dropout = nn.Dropout(dropout)
        self.att_scorer = TimeDistributed(output_size, 1)
        self.contextualized = False
        self.hidden_size = (input_size+6) // 2
        self.lstm = nn.LSTM(input_size+6, self.hidden_size, 1, dropout=dropout, bidirectional=True)

        self.dense = TimeDistributed(input_size, output_size)
        self.arg_dense = TimeDistributed(6,6)

    def forward(self, sentence_reps, arg_feature, sentence_mask, valid_scores):
        # x = self.dropout(x)
        # x = self.dense(x)
        # x = torch.tanh(x)
        # x = self.dropout(x)
        ...
        sentence_arg_reps = torch.cat([sentence_reps, arg], 2)
        h_0 = Variable(torch.zeros(2, sentence_arg_reps.size(1), self.hidden_size).cuda())
        c_0 = Variable(torch.zeros(2, sentence_arg_reps.size(1), self.hidden_size).cuda())
        # print(sentence_reps.shape)
        sent_embeddings = self.dropout(sentence_arg_reps)
        sentence_embedding, (_, _) = self.lstm(sent_embeddings, (h_0, c_0))
...
class ArgSentenceAttention2(nn.Module):
    """
    sentence-level attention. paragraph representation for label prediction.
    """

    def __init__(self, input_size, output_size, dropout=0.1):
        super(ArgSentenceAttention2, self).__init__()
        self.sentence_attention = TimeDistributed(input_size + 6, output_size)
        self.activation = torch.tanh
        self.dropout = nn.Dropout(dropout)
        self.att_scorer = TimeDistributed(output_size, 1)
    ...
    def forward(self, sentence_reps, arg_feature, sentence_mask, valid_scores):
        ...
        arg = self.dropout(arg)
        # print('x', x.shape, 'arg', arg.shape)
        sentence_arg_reps = torch.cat([x, arg], 2)
...
class AbstractRationaleJointModelClassifier(nn.Module):
    def __init__(self, args):
        super(AbstractRationaleJointModelClassifier, self).__init__()
        self.bert = AutoModel.from_pretrained(args.model)
        self.dropout = args.dropout
        self.hidden_dim = args.hidden_dim
        self.num_abstract_label = 3
    ...
    def forward(self, encoded_dict, transformation_indices, abstract_label=None, rationale_label=None,
        ...
        #paragraph_representations, sen_att_score = self.sentence_attention(sentence_representations, sentence_mask, valid_scores)
        #claim_paragraph = torch.mul(claim_representation, paragraph_representations)
        #abstract_score = self.abstract_linear(claim_paragraph)
        #abstract_out = torch.argmax(abstract_score.cpu(), dim=-1).detach().numpy().tolist()
        ...
        if rationale_label is not None:
            ...
            sentence_loss2 = self.retrieval_rationale_criterion(rationale_pred_label.view(-1),
                                                                sentence_att_scores.view(-1).detach())
            bce_loss = sentence_loss1 + sentence_loss2
            # bce_loss 就是正则化项rationale regularization
        else:
            rationale_loss = None
            bce_loss = None
        ...
        if train:
            return rationale_out, rationale_loss, retrieval_loss, bce_loss
        return rationale_out, retrieval_out


class ArgJointModelClassifier(nn.Module):
    def __init__(self, args):
        super(ArgJointModelClassifier, self).__init__()
        self.num_abstract_label = 3
        self.num_rationale_label = 2
        self.sim_label = 2
        self.bert = AutoModel.from_pretrained(args.model)
        ...
        self.word_attention = WordAttention(self.hidden_dim, self.hidden_dim, dropout=self.dropout)
        self.sentence_attention = SentenceAttention(self.hidden_dim, self.hidden_dim, dropout=self.dropout)
        self.sentence_attention2 = ArgSentenceAttention(self.hidden_dim, self.hidden_dim, dropout=self.dropout)
        self.rationale_linear = ClassificationHead(self.hidden_dim, self.num_rationale_label, dropout=self.dropout)
        self.abstract_linear = ClassificationHead(self.hidden_dim, self.num_abstract_label, dropout=self.dropout)
        self.abstract_retrieval = AbstractAttention(self.hidden_dim, self.sim_label, dropout=self.dropout)
        self.self_attention = SelfAttentionNetwork(self.hidden_dim, dropout=self.dropout)
        #self.arg_linear = ArgClassificationHead(self.hidden_dim, self.num_rationale_label, dropout=self.dropout)

        self.arg_feature_size = 6
        self.arg_linear = JointArgClassificationHead(self.hidden_dim, self.arg_feature_size, self.hidden_dim, self.num_rationale_label, hidden_dropout_prob=self.dropout)

        self.extra_modules = [
            self.word_attention,
            self.sentence_attention2,
            self.abstract_linear,
            self.rationale_linear,
            self.arg_linear,
            self.abstract_criterion,
    ...
    def forward(self, encoded_dict, transformation_indices, arg_feature, abstract_label=None, rationale_label=None,
                retrieval_label=None, train=False, retrieval_only=False, rationale_sample=1):
        batch_indices, indices_by_batch, mask = transformation_indices
        # match_batch_indices, match_indices_by_batch, match_mask = match_indices
        # (batch_size, num_sep, num_token)
        ...
        claim_sentence = torch.mul(claim_representation.unsqueeze(1), sentence_representations)
        #rationale_score = self.rationale_linear(claim_sentence)
        rationale_score = self.arg_linear(claim_sentence, arg_feature)  # (Batch_size, N_sep, 2)
        ...
        paragraph_representations, sen_att_score = self.sentence_attention2(sentence_representations, arg_feature,
                                                                           sentence_mask, valid_scores)
...
class JointArgClassificationHead(nn.Module):
    ...
    def __init__(self, hidden_size, arg_feature_size, sentence_repr_size, num_labels, hidden_dropout_prob = 0.1, return_sent_repr=False):
        ...
        self.dense = TimeDistributed(hidden_size, hidden_size)
        self.arg_dense = TimeDistributed(arg_feature_size, arg_feature_size)

        self.dropout = nn.Dropout(hidden_dropout_prob)
        self.out_proj = TimeDistributed(sentence_repr_size+arg_feature_size, num_labels)
...
class ArgJointParagraphClassifier(nn.Module):
    def __init__(self, bert_path, bert_dim, dropout=0.1, arg_feature_size=6, sentence_repr_size=1024, ignore_index=2):
        ...
        self.sentence_attention = SentenceAttention(bert_dim, bert_dim, dropout=dropout)
    ...
    def reinitialize(self):
        ...
        self.sentence_attention = SentenceAttention(self.bert_dim, self.bert_dim, dropout=self.dropout)
        self.extra_modules = [
            self.arg_linear,
            # self.rationale_linear,
            self.stance_linear,
