...
import numpy as np
from my_models import TestBertForSequenceClassification
...
def read_examples_by_abstract(claim, abstract):
    examples = []
    for i, sen in enumerate(abstract):
        #print(sen, claim)
        examples.append(InputExample(guid=i, text_a=sen, text_b=claim, label=None, role=None))
    return examples
...
def predict(model, tokenizer, args):
    ...
    for data in tqdm(list(dataset)):
        ...
        evidences = {}
        for doc_id in data['retrieved_doc_ids'][:args.k]:
            doc = corpus[int(doc_id)]
            sentences = doc['abstract']

            eval_examples = read_examples_by_abstract(claim, sentences)
            ...
            inference_labels = (logits[:, 0]>= 0.5).nonzero()[0].tolist() # nonzero()函数用来获取非0元素的索引

            #evidences[doc_id] = inference_labels
            evidences[doc_id] = logits[:, 0].tolist()
...
def main():
    ...
    parser.add_argument("--dataset", default='../../data/para_scifact/claims_train_tfidf_retrieved.jsonl', type=str)
    #parser.add_argument("--abstract_retrieval", default='abstract_retrieval.jsonl', type=str)
    parser.add_argument("--checkpoint", default='model/baseline/pytorch_model.bin', type=str)
    parser.add_argument("--output_path", default='k30_train_tfidf_scores_baseline_predicted_rationale_selection.jsonl', type=str)
