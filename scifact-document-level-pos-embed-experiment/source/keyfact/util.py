...
# 我裁断abstract之后，这里生成rationale预测结果的时候，要根据丢弃的句子id恢复正确的预测证据句id
# 原句子序列 ：0 1 2 3 4 5
# 裁断后，丢弃2 3 变成 0 1 4 5
# 但是预测的时候比如结果是 0010,则输出的rationale_id是2 [0 1 2 3]，但是原句子里的r_id应该是4,
# 所以这里应该存储以下原句id和现在句子id的对应词典 original_r_id = order_d[now_predict_id]
# order_d的构造方式应该是0:0 1:1 2:4 3:5 ,需要保留kept_sentence_ids，然后按现在的句子数挨个赋kept senid的值
def truncate_rationale2json(order_dicts, true_pairs, predictions, excluded_pairs=None):
    claim_ids = []
    claims = {}
    assert (len(true_pairs) == len(predictions))
    for pair, prediction,order in zip(true_pairs, predictions,order_dicts):
        ...
        for i, pred in enumerate(prediction):
            if pred == "rationale" or pred == 1:
                predicted_sentences.append(order[i])
