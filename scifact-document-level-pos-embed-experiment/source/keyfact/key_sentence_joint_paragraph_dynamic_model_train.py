...
if __name__ == "__main__":
    ...
    argparser.add_argument('--train_scores_dataset', type=str, default="k30_train_tfidf_scores_baseline_predicted_rationale_selection.jsonl")
    argparser.add_argument('--test_scores_dataset', type=str, default="k30_scores_baseline_predicted_rationale_selection.jsonl")
    ...
    train_set = RefinedAbstract_SciFactParagraphBatchDataset(corpus=args.corpus_file, claims=args.train_file,
                                                 scores_dataset=args.train_scores_dataset,
                                                 sep_token=tokenizer.sep_token, k=args.k,
                                                 downsample_n=args.downsample_n,
                                                 downsample_p=args.downsample_p)

    dev_set = RefinedAbstract_SciFactParagraphBatchDataset(corpus=args.corpus_file, claims=args.test_file,
                                                 scores_dataset=args.test_scores_dataset,
                                                 sep_token=tokenizer.sep_token, k=args.k,
                                                 downsample_n=0)
        ...
        for epoch in range(args.epoch):
            ...
            with open(output_eval_file, 'a', encoding='utf-8') as f:
                print(
                    ...
                    file=f)
                print(
                    f'Epoch {epoch}, dev stance f1 p r: %.4f, %.4f, %.4f, rationale f1 p r: %.4f, %.4f, %.4f' % dev_score,
                    file=f)
            ...
            if test:
                test_set = RefinedAbstract_SciFactParagraphBatchDataset(corpus=args.corpus_file, claims=args.test_file, scores_dataset=args.test_scores_dataset,
