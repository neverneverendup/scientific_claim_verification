...
def tokenize_sent(s):
    ...
    encoded_dict = tokenizer.batch_encode_plus(s, return_token_type_ids=False, return_attention_mask=False)
    a_len = [len(i) for i in encoded_dict['input_ids']]
    return a_len


class RefinedAbstract_SciFactParagraphBatchDataset(Dataset):
    """
    Dataset for a feeding a paragraph to a single BERT model.
    """
    def __init__(self, corpus: str, claims: str, scores_dataset, sep_token="</s>", k=0, train=True, dummy=True,
        ...
        scores_data = {}
        for did,data in enumerate(jsonlines.open(scores_dataset)):
            id = data['claim_id']
            scores_data[id] = data["evidence"]
        ...
        for claim in jsonlines.open(claims):
            ...
            for doc_id in all_candidates:
                ...
                kept_sentences_ids = []
                #print(claim["id"], doc_id)
                #print(scores_data[claim['id']].keys())
                if len(encoded_dict['input_ids']) > 512 and doc_id in scores_data[claim['id']].keys():
                    #scores = data['evidence'][doc_id]
                    scores = scores_data[claim['id']][doc_id]
                    temp_len = tokenize_sent(claim["claim"])
                    roles = evi_role[int(doc_id)]
                    a_len = tokenize_sent(corpus[int(doc_id)]['abstract'])
                    while temp_len < 512:
                        max_s = 0.0
                        for i, val in enumerate(scores):
                            if val > max_s and i not in kept_sentences_ids:
                                max_s = val
                                t_max_score_sen_id = i

                        if temp_len + a_len[t_max_score_sen_id] > 512:
                            break

                        temp_len += a_len[t_max_score_sen_id]
                        kept_sentences_ids.append(t_max_score_sen_id)
                        if len(kept_sentences_ids) == len(scores):
                            break
                else:
                    kept_sentences_ids = [i for i in range(len(corpus[int(doc_id)]['abstract']))]
                kept_sentences_ids = sorted(kept_sentences_ids)

                #print(len(corpus[int(doc_id)]['abstract']),len(kept_sentences_ids),kept_sentences_ids)

                this_role = [val for i,val in enumerate(roles) if i in kept_sentences_ids]
                ...
                if train:
                    for down_n in range(downsample_n + 1):
                        if doc_id in claim['evidence']:
                            ...
                            if down_n > 0:
                                abbstract_sentences, evidence_sentence_idx, stance, kept_s_ids, method_s_ids = \
                                    self.downsample(abstract_sentences, evidence_sentence_idx, stance, downsample_p, roles)

                                t = []
                                kept_sentences_ids = []
                                #  'claim_id': [1028], 'doc_id': [13923140],
                                for i in kept_s_ids.split('_'):
                                    if i == '_' or i =='':
                                        continue
                                    kept_sentences_ids.append(int(i.replace('_','')))


                                for i,val in enumerate(roles):
                                    if i in kept_sentences_ids:
                                        t.append(val)
                                this_role = t

                                if len(abbstract_sentences) == 0:
                                    break
                        ...
                        concat_sentences = (" " + sep_token + " ").join([val for i,val in enumerate(abstract_sentences) if i in kept_sentences_ids])
                        ...
                        rationale_label_string = "".join([val for i,val in enumerate(rationale_label_string) if i in kept_sentences_ids])
                    ...
                    if doc_id in claim['evidence']:
                        ...
                        for i in evidence_sentence_idx:
                            if i not in kept_sentences_ids:
                                error += 1
                                break

                    concat_sentences = (" " + sep_token + " ").join(
                        [val for i, val in enumerate(abstract_sentences) if i in kept_sentences_ids])
                    concat_sentences = clean_num(clean_url(concat_sentences))
...
# 首先用verisci获取关键句，组成短摘要，然后再用para编码
class RefinedAbstract_TestSciFactParagraphBatchDataset(Dataset):
    """
    Dataset for a feeding a paragraph to a single BERT model.
    """
    def __init__(self, corpus: str, claims, scores_dataset, sep_token="</s>", k=0, train=True, dummy=True,
                 downsample_n=0, downsample_p=0.5):
        self.label_ind = {"NEI": 0, "rationale": 1}
        self.rev_label_ind = {i: l for (l, i) in self.label_ind.items()}
        self.stance_ind = {"NEI": 0, "SUPPORT": 1, "CONTRADICT": 2}
        ...
        claim_dict = {}
        for claim in jsonlines.open(claims):
            claim_dataset[claim['id']] = claim['claim']
            claim_dict[claim['id']] = claim['evidence']
        #print(evi_role)
        error_case = 0
        for did,data in enumerate(jsonlines.open(scores_dataset)):
            id = data['claim_id']
            claim = claim_dataset[id]
            for doc_id in data['evidence'].keys():
                scores = data['evidence'][doc_id]
                collected_ids = []
                temp_len = tokenize_sent(claim)
                roles = evi_role[int(doc_id)]
                a_len = tokenize_sent(corpus[int(doc_id)]['abstract'])
                ...
                while temp_len < 512:
                    max_s = 0.0
                    for i,val in enumerate(scores):
                        if val > max_s and i not in collected_ids:
                            max_s = val
                            t_max_score_sen_id = i
                    ...
                    temp_len += a_len[t_max_score_sen_id]
                    collected_ids.append(t_max_score_sen_id)
                    if len(collected_ids) == len(scores):
                        break

                gold_rationales = []
                if doc_id in claim_dict[id].keys():
                    for group in claim_dict[id][doc_id]:
                        gold_rationales.extend(group["sentences"])

                #kept_sen_ids = [i for i in range(len(scores))]
                kept_sen_ids = sorted(collected_ids)
                d = [False for c in gold_rationales if c not in kept_sen_ids]
                if d:
                    error_case += 1

                this_role = [val for i, val in enumerate(roles) if i in kept_sen_ids]

                abstract_sentences = [val for i,val in enumerate(corpus[int(doc_id)]['abstract']) if i in kept_sen_ids]
                ...
                self.samples.append({
                    ...
                    'doc_id': doc_id,
                    'paragraph': concat_sentences,
                    'sentence_ids': kept_sen_ids,
                    'role': this_role
                })
        print('error_case_num', error_case)
