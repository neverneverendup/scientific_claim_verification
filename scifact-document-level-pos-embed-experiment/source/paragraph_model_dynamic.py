...
class WordAttention(nn.Module):
    ...
    def forward(self, x, token_mask):
        #print('x.size', x.shape, x.view(-1, x.size(-1)).shape)
        proj_input = self.att_proj(self.dropout(x.view(-1, x.size(-1))))
        #print('proj_input', proj_input.shape)
        ...
        #print('raw att scores', raw_att_scores.shape)
        ...
        #print('att scores', att_scores.shape)

        batch_att_scores = att_scores.view(-1, att_scores.size(-1)) # (Batch_size * N_sentence, N_token)
        #print(batch_att_scores.unsqueeze(1).shape, x.view(-1, x.size(2), x.size(3)).shape)
...
class ArgClassificationHead(nn.Module):
    ...
    def __init__(self, hidden_size, arg_feature_size, sentence_repr_size, num_labels, hidden_dropout_prob = 0.1):
        super().__init__()
        self.dense = TimeDistributedDense(hidden_size, 1024)
        ...
        self.dropout = nn.Dropout(hidden_dropout_prob)
        self.out_proj = TimeDistributedDense(1024+arg_feature_size, num_labels)
        #self.out_proj_x = TimeDistributedDense(sentence_repr_size, num_labels)
...
class ArgRationaleParagraphClassifier(nn.Module):
    def __init__(self, bert_path, bert_dim, dropout=0.1, arg_feature_size=5, sentence_repr_size=5, ignore_index=2):
    ...
    def reinitialize(self):
        self.extra_modules = []
        self.arg_linear = ClassificationHead(self.bert_dim, self.rationale_label_size,
