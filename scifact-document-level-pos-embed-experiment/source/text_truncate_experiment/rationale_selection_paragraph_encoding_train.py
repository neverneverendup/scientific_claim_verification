...
from my_models import ArgRationaleParagraphClassifier, RationaleParagraphClassifier
    ...
    with torch.no_grad():
        for batch in tqdm(DataLoader(dataset, batch_size = args.eval_batch_size, shuffle=False)):
            ...
            rationale_out, rationale_loss = \
                model(encoded_dict, transformation_indices, arg_feature,
...
def token_idx_by_sentence(input_ids, sep_token_id, model_name):
    ...
    #print(input_ids)
    sep_tokens = (input_ids == sep_token_id).bool()
    #print('setp tokens', sep_tokens)
    paragraph_lens = torch.sum(sep_tokens,1).numpy().tolist()
    #print('paragraph lens', paragraph_lens)
    indices = torch.arange(sep_tokens.size(-1)).unsqueeze(0).expand(sep_tokens.size(0),-1)
    ...
    #print('batch_indices',batch_indices.shape,'indices_by_batch',indices_by_batch.shape)
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--rationale_selection_model_type', type=str, default="arg", help="model type")
    ...
    argparser.add_argument('--use_ref_data', action='store_true', default=False)
    ...
    if train:
        train_set = SciFactParagraphBatchDataset(args.corpus_file, args.train_file, 
                                                 sep_token = tokenizer.sep_token, k = args.k, dummy=False, evaluation=False, use_ref_abstract=args.use_ref_data)
    dev_set = SciFactParagraphBatchDataset(args.corpus_file, args.test_file,
                                           sep_token=tokenizer.sep_token, k=args.k, dummy=False, evaluation=True, use_ref_abstract=False)
    evi_role = get_evi_role_dict()

    # 注意调用的时候，需要设置k值，不设置k值的话，系统会默认用cited-doc作为待抽取摘要
    # 设置大于0的k值之后，会使用retrived abstract。
    if args.rationale_selection_model_type == "arg":
        model = ArgRationaleParagraphClassifier(bert_path=args.repfile, bert_dim=args.bert_dim, dropout = args.dropout, arg_feature_size = len(role_type), sentence_repr_size = args.sent_repr_dim)
        print('model:arg')

    else:
        model = RationaleParagraphClassifier(bert_path=args.repfile, bert_dim=args.bert_dim, dropout = args.dropout)
        print('model:para')
    ...
    if train:
        ...
        for epoch in range(args.epoch):
            tq = tqdm(DataLoader(train_set, batch_size = args.train_batch_size, shuffle=False)) # shuffle=True
            ...
            with open(output_eval_file, "a") as f:
                ...
                print(f'Epoch {epoch}, train rationale f1 p r: %.4f, %.4f, %.4f' % train_score[:-1])
                print(f'Epoch {epoch}, eval rationale f1 p r: %.4f, %.4f, %.4f' % dev_score[:-1])
                ...
                if dev_perf >= prev_performance:
                    ...
                    print("Best eval score! New model saved!")