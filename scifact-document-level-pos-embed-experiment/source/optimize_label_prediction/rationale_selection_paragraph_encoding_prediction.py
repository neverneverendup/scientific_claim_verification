...
def predict(model, dataset):
    ...
    with torch.no_grad():
        for batch in tqdm(DataLoader(dataset, batch_size = args.eval_batch_size, shuffle=False)):
            ...
            rationale_out, _= model(encoded_dict,  transformation_indices, arg_feature)
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--checkpoint', type=str, default=10)
    ...
    rationale_selection_prediction_file = os.path.join(args.output_dir, 'rationale_selection.jsonl')
    ...
    dev_set = SciFactParagraphBatchDataset(args.corpus_file, args.test_file, train=False,
                                           sep_token=tokenizer.sep_token, k=args.k, dummy=False, evaluation=False, use_ref_abstract=False)
    print('samples', dev_set.samples)
    ...
    rationale_predictions = predict(model, dev_set)
    rationale_json = rationale2json(dev_set.samples, rationale_predictions)

    with jsonlines.open(rationale_selection_prediction_file, 'w') as output:
