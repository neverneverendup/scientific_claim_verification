...
def encode(tokenizer, batch, max_sent_len = 5000):
...
if __name__ == "__main__":
    ...
    lengths = []
    file = open('lengths_dev.txt','w',encoding='utf-8')
    if train:
        ...
        for epoch in range(args.epoch):
            ...
            for i, batch in enumerate(tq):
                ...
                #print(batch)
                #print(encoded_dict["input_ids"].shape[-1])
                lengths.append(encoded_dict["input_ids"].shape[-1])
            break
        print(len(lengths))
        for i in lengths:
            print(i, file=file)
