...
def predict(model, dataset):
    ...
    with torch.no_grad():
        for batch in tqdm(DataLoader(dataset, batch_size = args.eval_batch_size, shuffle=False)):
            ...
            rationale_out, stance_out, _, _ = model(encoded_dict, transformation_indices, arg_feature, rationale_score=True)
            print('rationale_out', rationale_out)
            values, indices = torch.topk(torch.tensor(rationale_out), k=3, dim=-1)  # (Batch_size, N_sep)
            print('values', values)
...
'''
    #dev_set = ArgSciFactParagraphBatchDataset(args.corpus_file, args.test_file, sep_token = tokenizer.sep_token, k = args.k, downsample_n=0)
    ...
    #checkpoint_dict = torch.load(args.args.checkpoint)
    ...
    if test:
        ...
        prediction_file = args.output_dir + '/joint_model_predictions_on_testset.jsonl'
