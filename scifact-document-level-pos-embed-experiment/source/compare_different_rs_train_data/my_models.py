...
class JointArgClassificationHead(nn.Module):
    ...
    def __init__(self, hidden_size, arg_feature_size, sentence_repr_size, num_labels, hidden_dropout_prob = 0.1, return_sent_repr=False):
        ...
        self.dense = TimeDistributedDense(hidden_size, sentence_repr_size)
        self.arg_dense = TimeDistributedDense(arg_feature_size, arg_feature_size)
    ...
    def forward(self, x, arg_feature, **kwargs):
        ...
        concated_x = torch.cat([sent_x, arg],2)

        out_x = self.out_proj(concated_x)
        #x = torch.tanh(x)
        #x = self.output(x)
...
class ArgJointParagraphClassifier(nn.Module):
    ...
    def forward(self, encoded_dict, transformation_indices, arg_feature, stance_label=None, rationale_label=None, sample_p=1,
        ...
        #rationale_out = self.rationale_linear(sentence_reps)  # (Batch_size, N_sep, 2)
        rationale_out, resized_sent_repr = self.arg_linear(sentence_reps, arg_feature)  # (Batch_size, N_sep, 2)
        ...
        if bool(torch.rand(1) < sample_p):  # Choose sentence according to predicted rationale
            ...
            valid_scores = rationale_label == 1  # Ground truth
        paragraph_rep = self.sentence_attention(sentence_reps, sentence_mask, att_scores, valid_scores)
        #paragraph_rep = self.sentence_attention(resized_sent_repr, sentence_mask, att_scores, valid_scores)
...
# joint arg model 改进调整，使用所有句子表示 按rationale得分加权获得文档表示
class AllRepArgJointParagraphClassifier(nn.Module):
    def __init__(self, bert_path, bert_dim, dropout=0.1, arg_feature_size=6, sentence_repr_size=1024, ignore_index=2):
        super(AllRepArgJointParagraphClassifier, self).__init__()
        self.stance_label_size = 3
        self.rationale_label_size = 2
        self.ignore_index = 2
        self.bert = AutoModel.from_pretrained(bert_path)
...
# 计算段落表示的时候 利用融合arg特征的句子表示
class ArgRepArgJointParagraphClassifier(nn.Module):
    def __init__(self, bert_path, bert_dim, dropout=0.1, arg_feature_size=6, sentence_repr_size=1024, ignore_index=2):
        super(ArgRepArgJointParagraphClassifier, self).__init__()
        self.stance_label_size = 3
        self.rationale_label_size = 2
        self.ignore_index = 2
        self.bert = AutoModel.from_pretrained(bert_path)
        ...
        self.dropout = dropout
        self.bert_dim = bert_dim
        self.sentence_attention = DynamicSentenceAttention(sentence_repr_size+arg_feature_size, bert_dim, dropout=dropout)
        self.word_attention = WordAttention(bert_dim, bert_dim, dropout=dropout)
        self.rationale_linear = ClassificationHead(self.bert_dim, self.rationale_label_size, hidden_dropout_prob=dropout)

        self.arg_feature_size = arg_feature_size
        self.sentence_repr_size = sentence_repr_size
        self.arg_linear = JointArgClassificationHead(bert_dim, self.arg_feature_size, self.sentence_repr_size, self.rationale_label_size, hidden_dropout_prob=dropout, return_sent_repr=True)

        self.stance_linear = ClassificationHead(sentence_repr_size+arg_feature_size, self.stance_label_size, hidden_dropout_prob=dropout)
        self.extra_modules = [
            self.sentence_attention,
            self.word_attention,
            # self.rationale_linear,
    ...
    def forward(self, encoded_dict, transformation_indices, arg_feature, stance_label=None, rationale_label=None, sample_p=1,
        ...
        #rationale_out = self.rationale_linear(sentence_reps)  # (Batch_size, N_sep, 2)
        rationale_out, arg_sent_repr = self.arg_linear(sentence_reps, arg_feature)  # (Batch_size, N_sep, 2)
        ...
        if bool(torch.rand(1) < sample_p):  # Choose sentence according to predicted rationale
            ...
            valid_scores = rationale_label == 1  # Ground truth
        #print('sen',sentence_reps.shape, 'arg sen ',arg_sent_repr.shape)
        paragraph_rep = self.sentence_attention(arg_sent_repr, sentence_mask, att_scores, valid_scores)
        #paragraph_rep = self.sentence_attention(sentence_reps, sentence_mask, sentence_mask.float(), sentence_mask)
        #print('para rep',paragraph_rep.shape)
...
# joint arg model 改进调整，使用所有句子表示 按rationale得分加权获得文档表示
class AvgRepArgJointParagraphClassifier(nn.Module):
    def __init__(self, bert_path, bert_dim, dropout=0.1, arg_feature_size=6, sentence_repr_size=1024, ignore_index=2):
        super(AvgRepArgJointParagraphClassifier, self).__init__()
        self.stance_label_size = 3
        self.rationale_label_size = 2
        self.ignore_index = 2
        self.bert = AutoModel.from_pretrained(bert_path)
    ...
    def forward(self, encoded_dict, transformation_indices, arg_feature, stance_label=None, rationale_label=None, sample_p=1,
        ...
        # 让挑选的证据句子的预测概率相同
        att_scores = torch.ones_like(att_scores, device='cuda')  # (BATCH_SIZE, N_sentence)
