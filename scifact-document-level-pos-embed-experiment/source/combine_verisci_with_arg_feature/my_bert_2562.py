...
from my_models import MyBertForSequenceClassification, TestBertForSequenceClassification
...
def read_examples(input_file, is_training):
    ...
    if 'test' not in input_file:
        df['label'] = df['categories'].map(label_cate2id)
    else:
        df['label'] = 0
    ...
    if 'train' in input_file:
        ...
        exm_num = 1000
    examples=[]
    for val in df[['paperid','abstract','title','label']].values[:exm_num,:]:
        #print(val)
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default='../model/roberta-large', type=str, required=False,
                        help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(ALL_MODELS))
    parser.add_argument("--meta_path", default=None, type=str, required=False,
                        help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(ALL_MODELS))
    parser.add_argument("--output_dir", default='../model/roberta_large_5121/roberta_large_5121_0', type=str, required=False,
    ...
    parser.add_argument("--freeze", default=0, type=int, required=False,
                        help="freeze bert.")
    parser.add_argument("--not_do_eval_steps", default=0.40, type=float,
    ...
    # config = BertConfig.from_pretrained("roberta-large", num_labels=39)
    # Prepare model
    ...
    if args.do_train:
        ...
        for e in range(args.epochs):
            model.train()
            t = tqdm(DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size))
            for i, batch in enumerate(t):
                ...
                if (i + 1) % (args.eval_steps * args.gradient_accumulation_steps) == 0:
                    tr_loss = 0
                    nb_tr_examples, nb_tr_steps = 0, 0
    ...
    #     #tokenizer = AutoTokenizer.from_pretrained("roberta-large")
    #     # Prepare model
    #     model = MyBertForSequenceClassification(rep_file="roberta-large", args=args)
    ...
    #         print('total logits', logits.shape)
    #         if flag=='test':
    #             df=pd.read_csv(os.path.join(args.data_dir, file))
    #             label_idxs = ['paperid']
    #             for label_idx in range(39):
    #                 df['label_'+str(label_idx)] = logits[:,label_idx]
    #                 label_idxs.append('label_'+str(label_idx))
    #
    #             df[label_idxs].to_csv(os.path.join(args.output_dir, "sub.csv"),index=False)
    #         if flag == 'dev':
    #             df = pd.read_csv(os.path.join(args.data_dir, file))
    #
    ...
    #                 label_idxs.append('label_' + str(label_idx))
    #
    #             df[label_idxs].to_csv(os.path.join(args.output_dir, "sub_dev.csv"),
    #                                                                index=False)
