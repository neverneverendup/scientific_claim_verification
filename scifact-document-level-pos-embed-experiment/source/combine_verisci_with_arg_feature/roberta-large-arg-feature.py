...
import numpy as np
from my_models import TestBertForSequenceClassification, ArgBertForSequenceClassification
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, split_num,
    ...
    for example_index, example in enumerate(examples):
        ...
        label = example.label
        role = example.role
        one_hot_role = ([0.0] * len(role_type))
        one_hot_role[role_type.index(role)] = 1.0
...
def evaluate(model, file, tokenizer, args):
    ...
    all_roles = torch.tensor([f.role for f in eval_features], dtype=torch.float32)
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label,all_roles)
    ...
    for input_ids, input_mask, segment_ids, label_ids, arg_feature in eval_dataloader:
        input_ids = input_ids.to(device)
        input_mask = input_mask.to(device)
        segment_ids = segment_ids.to(device)
        label_ids = label_ids.to(device)
        arg_feature = arg_feature.to(device)

        # input_ids = input_ids.view(-1,input_ids.size(-1))
        # input_mask = input_mask.view(-1,input_mask.size(-1))

        with torch.no_grad():
            loss, logits  = model(input_ids=input_ids, attention_mask=input_mask, labels=label_ids, arg_feature=arg_feature)
...
def main():
    ...
    all_roles = torch.tensor([f.role for f in train_features], dtype=torch.float32)
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label,all_roles)
    model.to(device)
    best_acc = 0.0
    for e in range(args.epochs):
        ...
        for i, batch in enumerate(t):
            ...
            input_ids, input_mask, segment_ids, label_ids, arg_feature = batch
            #print(label_ids)
            #print(arg_feature)
            # input_ids = input_ids.view(-1, input_ids.size(-1))
            # input_mask = input_mask.view(-1,input_mask.size(-1))
            loss, _ = model(input_ids=input_ids,  attention_mask=input_mask,
                            labels=label_ids, arg_feature=arg_feature)
            # loss = outputs.loss
            loss.backward()
        ...
        if dev_score[0] > best_acc :
            ...
            output_model_file = os.path.join('model/argfeature/', "pytorch_model.bin")
